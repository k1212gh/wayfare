...
async def upload_apk(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Cleanup partial file AND the just-created tour_dir to avoid orphan workspaces
...
async def upload_split_apks(files: list[UploadFile] = File(...)):
    ...
    return {"tour_id": tour_id, "status": "uploaded", "filename": f"base.apk (+{len(validated)-1} splits)"}
...
@app.post("/api/tours/{tour_id}/plan")
...
async def get_cache_stats(tour_id: str):
    ...
    Note: The pattern cache is GLOBAL (cross-tour). `tour_id` in the path is not
...
# `_SAFE_ID`, `_safe_tour_dir`, `_within` moved to paths.py (re-exported at top).
...
async def get_screenshot(tour_id: str, screen_id: str):
    ...
    # Strategy 1: Look up screenshot_ref from the graph (must be within tour_dir)
    graph_path = tour_dir / "output" / "screen_map.json"
    ...
    # Strategy 2: Search in known directories (result must be within search_dir)
    ...
    # Strategy 3: First available screenshot as fallback
    states_dir = tour_dir / "dynamic" / "states"
    if states_dir.exists():
